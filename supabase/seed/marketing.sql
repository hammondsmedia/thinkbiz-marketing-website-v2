...
VALUES
  ('garrett-hammonds@thinkbiz-seed.local', 'garrett-hammonds', 'Garrett', 'Hammonds',
   'Hammonds Media', 'Founder', 'Web Design & Digital Marketing', 'President',
  ...
  ('sarah-martinez@thinkbiz-seed.local', 'sarah-martinez', 'Sarah', 'Martinez',
   'Martinez Accounting Group', 'CPA', 'Accounting / CPA', 'Vice President',
  ...
  ('james-whitfield@thinkbiz-seed.local', 'james-whitfield', 'James', 'Whitfield',
   'Whitfield Business Law', 'Attorney', 'Attorney — Business Law', 'Member',
  ...
  ('priya-patel@thinkbiz-seed.local', 'priya-patel', 'Priya', 'Patel',
   'Patel Insurance Solutions', 'Broker', 'Business & Commercial Insurance', 'Member',
  ...
  ('brandon-cole@thinkbiz-seed.local', 'brandon-cole', 'Brandon', 'Cole',
   'Cole Realty Group', 'Realtor', 'Real Estate — Residential', 'Member',
  ...
  ('lisa-tran@thinkbiz-seed.local', 'lisa-tran', 'Lisa', 'Tran',
   'Tran Mortgage Group', 'Mortgage Loan Officer', 'Mortgage / Home Loans', 'Secretary',
  ...
  ('rachel-kim@thinkbiz-seed.local', 'rachel-kim', 'Rachel', 'Kim',
   'Kim Financial Planning', 'CFP', 'Financial Planning & Wealth Management', 'Member',
  ...
  ('david-nguyen@thinkbiz-seed.local', 'david-nguyen', 'David', 'Nguyen',
   'Nguyen Roofing & Exteriors', 'Owner', 'Roofing & Exterior Services', 'President',
  ...
  ('amara-williams@thinkbiz-seed.local', 'amara-williams', 'Amara', 'Williams',
   'Williams Estate Law', 'Attorney', 'Attorney — Estate Planning', 'Vice President',
  ...
  ('carlos-mendez@thinkbiz-seed.local', 'carlos-mendez', 'Carlos', 'Mendez',
   'Mendez Financial Group', 'Insurance Specialist', 'Life & Health Insurance', 'Member',
  ...
  ('jennifer-brooks@thinkbiz-seed.local', 'jennifer-brooks', 'Jennifer', 'Brooks',
   'Brooks HR Consulting', 'HR Consultant', 'HR & Payroll Services', 'Member',
  ...
  ('mike-patterson@thinkbiz-seed.local', 'mike-patterson', 'Mike', 'Patterson',
   'Patterson Home Services', 'Owner', 'HVAC & Plumbing', 'President',
  ...
  ('taylor-reed@thinkbiz-seed.local', 'taylor-reed', 'Taylor', 'Reed',
   'Reed Photography & Video', 'Photographer', 'Photography & Videography', 'Member',
...
SELECT c.id, v.industry, v.industry_slug, v.status,
       ...
       v.sort_order
FROM (VALUES
  ('Web Design & Digital Marketing', 'web-design-digital-marketing', 'filled', 'garrett-hammonds@thinkbiz-seed.local', 1),
  ('Accounting / CPA', 'accounting-cpa', 'filled', 'sarah-martinez@thinkbiz-seed.local', 2),
  ('Attorney — Business Law', 'attorney-business-law', 'filled', 'james-whitfield@thinkbiz-seed.local', 3),
  ('Business & Commercial Insurance', 'business-commercial-insurance', 'filled', 'priya-patel@thinkbiz-seed.local', 4),
  ('Real Estate — Residential', 'real-estate-residential', 'filled', 'brandon-cole@thinkbiz-seed.local', 5),
  ('Mortgage / Home Loans', 'mortgage-home-loans', 'filled', 'lisa-tran@thinkbiz-seed.local', 6),
  ('Financial Planning & Wealth Management', 'financial-planning-wealth-management', 'filled', 'rachel-kim@thinkbiz-seed.local', 7),
  ('HVAC & Plumbing', 'hvac-plumbing', 'open', NULL, 8),
  ('Recruiting & Staffing', 'recruiting-staffing', 'open', NULL, 9),
  ('Commercial Real Estate', 'commercial-real-estate', 'open', NULL, 10),
  ('Physical Therapy / Chiropractic', 'physical-therapy-chiropractic', 'open', NULL, 11),
  ('Title & Escrow', 'title-escrow', 'open', NULL, 12)
...
SELECT c.id, v.industry, v.industry_slug, v.status,
       ...
       v.sort_order
FROM (VALUES
  ('Roofing & Exterior Services', 'roofing-exterior-services', 'filled', 'david-nguyen@thinkbiz-seed.local', 1),
  ('Attorney — Estate Planning', 'attorney-estate-planning', 'filled', 'amara-williams@thinkbiz-seed.local', 2),
  ('Life & Health Insurance', 'life-health-insurance', 'filled', 'carlos-mendez@thinkbiz-seed.local', 3),
  ('HR & Payroll Services', 'hr-payroll-services', 'filled', 'jennifer-brooks@thinkbiz-seed.local', 4),
  ('Accounting / CPA', 'accounting-cpa', 'open', NULL, 5),
  ('Mortgage / Home Loans', 'mortgage-home-loans', 'open', NULL, 6),
  ('Real Estate — Residential', 'real-estate-residential', 'open', NULL, 7),
  ('Web Design & Digital Marketing', 'web-design-digital-marketing', 'open', NULL, 8),
  ('Financial Planning & Wealth Management', 'financial-planning-wealth-management', 'open', NULL, 9),
  ('Commercial Cleaning', 'commercial-cleaning', 'open', NULL, 10)
) AS v(industry, industry_slug, status, member_email, sort_order),
     public.clubs c
...
SELECT c.id, v.industry, v.industry_slug, v.status,
       ...
       v.sort_order
FROM (VALUES
  ('HVAC & Plumbing', 'hvac-plumbing', 'filled', 'mike-patterson@thinkbiz-seed.local', 1),
  ('Photography & Videography', 'photography-videography', 'filled', 'taylor-reed@thinkbiz-seed.local', 2),
  ('Accounting / CPA', 'accounting-cpa', 'open', NULL, 3),
  ('Attorney — Business Law', 'attorney-business-law', 'open', NULL, 4),
  ('Business & Commercial Insurance', 'business-commercial-insurance', 'open', NULL, 5),
  ('Real Estate — Residential', 'real-estate-residential', 'open', NULL, 6),
  ('Mortgage / Home Loans', 'mortgage-home-loans', 'open', NULL, 7),
  ('Financial Planning & Wealth Management', 'financial-planning-wealth-management', 'open', NULL, 8),
  ('Landscaping & Lawn Care', 'landscaping-lawn-care', 'open', NULL, 9),
  ('Personal Training / Fitness', 'personal-training-fitness', 'open', NULL, 10)
) AS v(industry, industry_slug, status, member_email, sort_order),
     public.clubs c
WHERE c.slug = 'edmond'
ON CONFLICT (club_id, industry_slug) DO UPDATE SET
